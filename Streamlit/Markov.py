...
# ============================================================
# 0) Estilo (CSS) para que se vea más bonito para estudiantes
# ============================================================
CSS = """
<style>
html, body, [class*="css"]  {
...
# ============================================================
# 1) Modelo OOP: Cadena de Markov de 2 estados (V/C)
# ============================================================
...
# ============================================================
# 3) Grafo ponderado en Altair con FLECHAS (arrowheads)
# ============================================================
...
def df_graph(chain: TwoStateMarkovChain):
    ...
    # Más separación horizontal + un pequeño "aire" vertical para etiquetas/loops
    nodes = pd.DataFrame({
        "id": [0, 1],
        "label": [labs[0], labs[1]],
        "x": [0.0, 1.45],
    ...
    def add_directed_curve(src, dst, bump, weight):
        ...
        # Etiqueta del peso: cerca del 55% (evita que se “pegue” al nodo)
        ...
        # Flecha: al 97% de la curva, con orientación tangente
    ...
    # Aristas entre nodos: curvas opuestas
    add_directed_curve(0, 1, bump=+0.24, weight=float(P[0, 1]))
    add_directed_curve(1, 0, bump=-0.24, weight=float(P[1, 0]))

    # Loops (auto-transiciones) como círculos discretizados con flecha pequeña
    def add_loop(node_id, weight, radius=0.22, ylift=0.30, xshift=0.0):
        ...
        # Flecha del loop: en theta ~ 20° (con tangente)
        k = int(0.10 * (len(xs) - 1))
        kprev = max(0, k - 2)
...
def chart_graph(chain: TwoStateMarkovChain) -> alt.Chart:
    ...
    # Arrowheads: triángulos con rotación (angle)
...
# ============================================================
# 4) Charts Altair (sin LaTeX en títulos/axis: Altair no lo renderiza)
# ============================================================
...
# ============================================================
# 5) App Streamlit (Markdown + LaTeX sin errores)
# ============================================================
...
# Render legible del fragmento en ruso
st.markdown(
    """
...
# Fórmulas: SOLO st.latex (evita “errores de LaTeX” en markdown)
...
with tab1:
    ...
    # Para evitar encimarse: en FILAS y con altura grande
    st.altair_chart(chart_heatmap_P(chain), use_container_width=True)
    st.markdown('<div class="colgap"></div>', unsafe_allow_html=True)
    st.altair_chart(chart_graph(chain), use_container_width=True)

with tab2:
    st.markdown("### Convergencia de la marginal")

    st.markdown('<div class="colgap"></div>', unsafe_allow_html=True)
    st.altair_chart(chart_marginal(chain, n_steps=min(2500, n_steps), x0=x0), use_container_width=True)
...
with tab3:
    ...
    with left:
        st.altair_chart(chart_sample_path(sim, max_points=1500), use_container_width=True)
    with right:
        st.altair_chart(chart_empirical_frequency(sim, max_points=6000), use_container_width=True)
...
""",
    ...
    left, right = st.columns(2, gap="large")
    with left:
        st.altair_chart(chart_transition_bars(sim), use_container_width=True)
    with right:
        c = sim.transition_counts()
        labs = chain.labels
